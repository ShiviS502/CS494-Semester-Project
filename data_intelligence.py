"""
Data Intelligence Agent — RAG retrieval + sentiment analysis.

Uses GPT-3.5-turbo to generate multi-angle search queries and perform
structured sentiment analysis. Retrieves relevant documents from ChromaDB;
falls back to raw_feedback when the key or collection is unavailable.
"""
...
def _heuristic_sentiment(docs: list[dict]) -> dict:
    """Simple keyword-based sentiment fallback (no API key needed)."""
...
def data_intelligence_node(state: CustomerIntelState) -> CustomerIntelState:
    ...
    Responsibilities:
    - Generate multiple search queries from the user's question (LLM)
    - Retrieve top-K relevant documents from ChromaDB via hybrid search
    - Perform sentiment analysis and theme clustering (LLM)
    ...
    if OPENAI_API_KEY:
        try:
            ...
            # Fall back to raw feedback if ChromaDB returned nothing
            ...
            # ── Step 3: Sentiment + theme analysis via LLM ────────────────────
            ...
            # Tag individual docs with heuristic sentiment for downstream use
            ...
            print(f"[Data Intelligence] LLM path failed ({e}). Using heuristic fallback.")
